...
import { getUserProfile } from '../models/profile.model';
...
export const getProfile = async (req: Request, res: Response) => {
  try {
    ...
    return apiResponse(res, 200, 'Sukses', result.rows[0]);
  } catch (error) {
    console.error(error);
...
export const updateProfile = async (req: Request, res: Response) => {
  try {
    ...
    return apiResponse(res, 200, 'Update Pofile berhasil', {
      email: user.email,
      first_name: user.first_name,
      last_name: user.last_name,
      profile_image: user.profile_image ? `https://yoururlapi.com/${user.profile_image}` : null,
    });
  } catch (error) {
    console.error(error);
    return apiResponse(res, 500, 'Internal Server Error', null);
